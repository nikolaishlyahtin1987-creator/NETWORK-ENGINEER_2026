...
#### На S1 корневой порт – Fa0/3, на S2 корневой порт – Fa0/3.       

#### **Почему протокол STP выбрал эти порты в качестве портов корневого моста на этих коммутаторах?**     
#### Все порты имеют одинаковую стоимость (19) и приоритет (128). Выбран порт, ведущий к коммутатору с наименьшим BID (корневому мосту) – в данном случае это порт Fa0/3, подключённый напрямую к S3.   
...
#### Идентификатор моста (Bridge ID) соседнего коммутатора.
#### Если стоимости путей равны, STP выбирает порт, ведущий к коммутатору с наименьшим BID (приоритет + MAC-адрес).      

#### **3. Если оба значения на двух портах равны, каким будет следующее значение, которое использует протокол STP при выборе порта?**       
#### Приоритет порта (port priority), а при его равенстве – номер порта.
#### STP сравнивает идентификаторы портов (port ID), которые состоят из приоритета порта (по умолчанию 128) и номера порта. Меньшее значение идентификатора порта является предпочтительным.   

[Скачать конфиг лабораторной работы №7](https://github.com/nikolaishlyahtin1987-creator/NETWORK-ENGINEER_2026/blob/main/Лабораторные%20работы/Лабораторная%20работа%20№7/Config_Лабораторная%20работа%20№7.pkt "Нажмите для скачивания файла конфигурации")
